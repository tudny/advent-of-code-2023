...
lazy_static! {
    ...
    static ref LIMIT_MAP: HashMap<&'static str, usize> = HashMap::from([("red", RED_LIMIT), ("green", GREEN_LIMIT), ("blue", BLUE_LIMIT)]);
}
...
fn check_single_line(line: String) -> Option<usize> {
    ...
    return if cap["draws"].split("; ").map(validate_draw).all(|x| x.is_some_and(|x| x)) {
        Some(game_id)
    } else {
        Some(0)
    };
